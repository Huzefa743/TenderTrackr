...
function Serviceaddresponse() {
  ...
  const [defaultcode, setDefaultCode] = useState(undefined);
  const [defaultcodeexist, setDefaultCodeExist] = useState(false);
  ...
  //add response api call
  const addresponse = async (event) => {
    setSubmitSpinner(true);
    event.preventDefault();
    if (isform === true) {
      await API.AddServiceResponse(
        collectionid,
        serviceid,
        statuscode,
        defaultcode,
      ...
      <Container className="container-box">
        <div className="mt-3 text-primary fw-bold">
          <Breadcrumb>
            ...
            <Breadcrumb.Item active>Add Response</Breadcrumb.Item>
          </Breadcrumb>
        </div>
        <Card>
          <Card.Header>
          <h5 className="fw-bold">Add Response</h5>
...
export default Serviceaddresponse;
