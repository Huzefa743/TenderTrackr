...
export default function QuotationForm(props) {
  ...
  const [quotationFormFields, setQuotationFormFileds] = useState({
    ...
    warrantyAfterDispatch:'',
    warrantyAfterCommissioning:'',
    ratingKva:'',
    noOfPhases:'',
    primaryVoltage:'',
    secondaryVoltage:'',
    ...
    cooling:'',
    frequency:'',
    tempInOilByThermometer:'',
    tempInWindingByResistance:'',
    losses50:'',
    losses50Unit:'',
    losses100:'',
    losses100Unit:'',
  ...
  const addAnnexureAForm =() =>{
   ...
   const removeAnnexureAForm =(index)=>{
     ...
     console.log("details", details.quotation_product_id)
     deleteQuotationProduct(details.quotation_product_id)
  ...
  async function deleteQuotationProduct(quotationProductId){
        API.deleteQuotationProduct(quotationProductId)
        .then((res) => {
           
         console.log("Deleted quotation product ", res)
        
    
        })
        .catch((err) => {
          console.log("delete quotation product failed!! ", err)
      })
  ...
  const createQuotation=async()=>{
     ...
     async function updateHandler(){
        setLoadSelect(true)
        API.updateQuotationById(quotationid, quotationFormFields)
        .then(async(res) => {
         console.log("here is update sop details ", res)
         if(res.status==200){
        ...
        .catch((err) => {
            setTimeout(() => {  setLoaderVisible(false) }, 2000);
            alert("update quotation not able to update!!")
            console.log("here is the quotation update failer stats!! ", err)
      })
        
     }

     async function getQuotationDetails(){
        API.getQuotationById(quotationid).then((res) => {
              console.log("quotation details", res)
              if(res.status==200){
                    setQuotationFormFileds(res.data.data)
                    setAnnexureAFormFileds(res.data.data.products)
              }
             })
             .catch((err) => {
               console.log("fetch quotatin details got error", err)
           })
      }
      useEffect(() => {
        getQuotationDetails()
  ...
  return (
   <div>
    ...
    <p style={{fontSize:24, fontWeight:700, margin:0, padding:0, marginBottom:10}}>Update Quotation</p>
    <div className='row' style={{height:'auto', backgroundColor:'', borderRadius:5, border:'1px solid lightgray', padding:0, margin:0, padding:10, marginBottom:20, paddingBottom:30}}>
          ...
          {annexureAFormFields.map((annexureAFields, index)=>(
            < div style={{border:'1px solid lightgray', width:'96%', borderRadius:5, padding:10, backgroundColor:'#FAFAFA', marginTop:20, marginLeft:20, marginRight:20, paddingBottom:30}}>
              ...
              <div className='row' style={{marginLeft:0}}>
                  ...
                  <div className='col-3'>
                       <TextField  margin="dense" name='unit_ex_works_price'  disabled={disableQuotationField} onChange={event => handleAnnexureAFormChange(event, index)} value={annexureAFields.unit_ex_works_price} id="name" label="Unit Ex-Works Price" placeholder='680000' type="number" fullWidth variant="standard"/>
                  </div>
                  <div className='col-3' >
              ...
              <div className='row' style={{marginLeft:0}}>
                    <div className='col-3'>
                    <TextField  margin="dense" name='f_and_i'  disabled={disableQuotationField} onChange={event => handleAnnexureAFormChange(event, index)} value={annexureAFields.f_and_i} id="name" label="F & I" placeholder='Extra' type="text" multiline  fullWidth variant="standard"/>
    
                    </div>
                  <div className='col-3'>
                       <TextField  margin="dense" name='grand_total'  disabled={disableQuotationField} onChange={event => handleAnnexureAFormChange(event, index)} value={annexureAFields.grand_total} id="name" label="Grand Total" placeholder='802400' type="number" fullWidth variant="standard"/>
                  </div>
                  </div>
          ...
          <p style={{fontWeight:700, color:'lightgray', paddingLeft:22, marginTop:20, marginBottom:0}}>Select the Load</p>
                            ...
                            <RadioGroup
                                ...
                                name="loadSelect" onChange={event => (handleQuotationFormChange(event), setLoadSelect(event.target.value) )} value={quotationFormFields.loadSelect} 
    ...
    <div className="row" style={{display:'flex', justifyContent:'flex-end', marginRight:0, marginBottom:20}}>
     ...
     <Button  onClick={()=>updateHandler()} disabled={disableQuotationField} style={{backgroundColor:'#0059BF', height:30, width:90, textAlign:'center', padding:0, fontSize:12, fontWeight:600, borderRadius:3, boxShadow:'0px 4px 4px rgba(0, 0, 0, 0.25)', border:'0px', color:'white'}}>Update</Button>
